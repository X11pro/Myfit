...
try {
  & flutter build apk --debug --dart-define-from-file=$definesPath
  if ($LASTEXITCODE -ne 0) {
    throw 'Fallo flutter build apk --debug.'
  }
} finally {
  Pop-Location
}

"APK generado en: $appDir\build\app\outputs\flutter-apk\app-debug.apk"
