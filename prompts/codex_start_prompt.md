# Prompt inicial para agente

Usa este prompt al abrir el repositorio en el nuevo sistema:

```text
Lee primero AGENTS.md, docs/product/fitness_product_plan.md y docs/handoff/current_status.md.

Quiero continuar el proyecto Myfit desde el estado actual del repo.

Objetivo inmediato:
1. Revisar el estado real de mobile/fitness_app y backend/supabase/migrations.
2. Crear un ExecPlan para conectar Supabase Auth y persistir el onboarding en profiles.
3. Implementar en pasos pequenos.
4. Mantener respuestas en espanol.
5. Ejecutar cuando esten disponibles: flutter pub get, dart format ., flutter analyze y flutter test.

No reinicies el proyecto desde cero. Continua desde la estructura y commits ya existentes.
```
